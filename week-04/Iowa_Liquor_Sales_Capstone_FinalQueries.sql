...
WHERE vendor LIKE 'Diageo%'
	...
	AND bottle_qty > 12
	ORDER BY total 
-- 6,780 sales with a bottle quantity greater than 12 in Q4,2014
...
-- 6 Variations of Captain Morgan
...
-- over $45 million in total revenue for 2014 in Iowa. (Bird's eye view)
...
-- Bacardi USA, biggest competitor with $6,789,296.58 revenue
-- Luxco St Louis, is 2nd with $2,876,411.12 revenue
...
-- Pernod Ricard with $3,718,889.70 revenue
-- Bacardi 2nd with $2,395,836.03 revenue
-- Jim Beam 3rd with $537,825.30 revenue
-- Diageo close in 4th with $441,450.60 revenue
...
ORDER BY total_revenue DESC;
-- 1,255 stores with total revenue of >$2000
...
GROUP BY category, category_name
ORDER BY total_revenue DESC;
-- Spiced Rum as a category, dominates with $28,261,863.71 in total revenue (for all vendors) and 146,947 transactions
...
SELECT s.store, st.name AS store_name, st.store_address, s.county,
    COUNT(*) AS total_transactions,
    SUM(s.bottle_qty) AS total_bottles_sold,
    SUM(s.total::money) AS total_revenue,
    ROUND(AVG(s.btl_price::numeric), 2) AS avg_bottle_price
...
WHERE s.vendor LIKE '%Diageo%'
  AND s.date BETWEEN '2014-01-01' AND '2014-12-31'
GROUP BY s.store, st.name, st.store_address, s.county
ORDER BY total_revenue DESC;
...
ORDER BY total_revenue DESC;
-- highest revenue per capita goes to Dickinson with a 27.34 
-- with only $455,691.69 in revenue and 16,667 population
-- The lowest revenue per capita goes to Fremont with a 0.47
-- with only $3,500.49 in revenue and 7,441 population
-- Most revenue goes to Polk county with $4,952,483.60 in revenue and 11.50 revenue per capita
...
ORDER BY total_revenue DESC;
-- #1 Polk County with $17,205,406.69 in revenue and 90,230 transactions, 1,002,961 bottles sold
-- Polk has a 430,640 population (makes sense why its the best) 11.50 revenue per capita
-- #2 Linn $6,322,545.66 and 37,551 transactions population, 377,520 bottles sold
-- Linn has a 211,226 population, 11.63 revenue per capita
-- #3 Scott $5,306,327.57 and 28,359 transactions , 322,722 bottles sold
-- Scott has a 165,224 population, 10.10 revenue per capita
...
ORDER BY total_revenue DESC;
-- No surprise here 
-- #1 Captain Morgan Spiced Rum with $747,405.00 revenue
-- #2 Crown Royal Canadian Whisky with $656,758.98 revenue
-- #3 Ketel One Imported Vodka with $243,449.76 revenue
-- 
